...
rm(list=ls())
require(pacman)
p_load('tidyverse', 'ggplot2', 'ggpubr', 'ggridges', 'ggsignif', 'here', 'lmerTest', 'car', 'psych', 'performance', 'corrplot', 'sjPlot', 'papaja', 'stringdist', 'wordcloud', 'MASS', 'pwr', 'png', 'grid', 'ltm') # removed: GGally::ggpairs(), effectsize::cohens_d()
...
<!-- 150 words, SGS limits lay abstract to 150 words -->
...
<!-- 294 words, SGS limits Abstract to 300 words -->
...
### Software
All analyses were performed in R 4.4.1 [@R-base], using custom scripts and third-party packages. Data and the code required to reproduce everything in this document [created with *papaja*, @R-papaja] is available at https://github.com/beatlab-mcmaster/Schlichting_MSc_Thesis. Required packages are detailed in the document source code, and below, alongside the relevant analysis. All figures are created with *ggplot2* [@R-ggplot2], *ggpubr* [@R-ggpubr], and *png* [@R-png] unless stated otherwise. Error bars represent 95% confidence intervals (CI) based on 1000 bootstrap resamples implemented in *Hmisc* [@R-Hmisc].
...
dv_corr("problem_awareness_z", control_vars, innocents_dichotom, output = "table") 
...
# awareness_lmm <- lmer(problem_awareness_z ~ base_problem_awareness_z + attendance * medium_2 * timepoint_2 + (1|p_id), data = innocents_2long)
...
# 
# awareness_log <- lmm_analysis(dv = "problem_awareness_log", formula = "attendance * medium_2 * timepoint_2", data = innocents_2long, control_vars = "base_problem_awareness_z")
...
# 
# awareness_recpr <- lmm_analysis(dv = "problem_awareness_recpr", formula = "attendance * medium_2 * timepoint_2", data = innocents_2long, control_vars = "base_problem_awareness_z")
...
# # Find the indices of the most extreme residuals
# order(abs(awareness_intercepts$`(Intercept)`), decreasing = TRUE)[1:2] # indices: c(85, 27)
# rownames(awareness_intercepts)[c(85, 27)] # p_id = c("sub-on107", "sub-in028")
# 
# rm(awareness_intercepts)

# Excluding the two outlier intercepts fixed the assumptions
awareness_lmm <- lmer(problem_awareness_z ~ base_problem_awareness_z + attendance * medium_2 * timepoint_2 + (1|p_id), data = subset(innocents_2long, !(p_id %in% c("sub-on107", "sub-in028")) & !is.na(problem_awareness_z)))
...
awareness_cov <- lmer(problem_awareness_z ~ base_problem_awareness_z + (1|p_id), data = subset(innocents_2long, !(p_id %in% c("sub-on107", "sub-in028")) & !is.na(problem_awareness_z)))
...
model_comp_table <- matrix(ncol = 2, nrow = 0) %>% # initiate table structure
  ...
  rbind(c("  Base Model", "Awareness \\textasciitilde Baseline + (1|Participant)")) %>%
  rbind(c("  Full Model", "Awareness \\textasciitilde Baseline + Attendance * Medium * Time + (1|Participant)")) %>%
...
### Awareness
Audiences already had a high problem awareness before the event, which increased slightly on average ($M = `r printnum(mean(outcomes_change$problem_awareness_c, na.rm = T), digits = 2)`$, $SD = `r printnum(sd(outcomes_change$problem_awareness_c, na.rm = T), digits = 2)`$, see Figure \@ref(fig:awareness-plots) A). The variance explained by the baseline alone was $R_{m}^2 = `r printnum(r2_nakagawa(awareness_cov)[["R2_marginal"]])`$, which together with the fixed effects increased to $R_{m}^2 = `r printnum(r2_nakagawa(awareness_lmm)[["R2_marginal"]])`$. The random intercept variance was $\sigma = `r printnum(VarCorr(awareness_lmm)$p_id[1], digits = 2)`$, summing to a total explained variance of $R_{c}^2 = `r printnum(r2_nakagawa(awareness_lmm)[["R2_conditional"]])`$.
...
(ref:awareness-plots) A: Mean and CI of the change in problem awareness from baseline to post-event by attendance mode. Individual data points are represented by dots. B: Mean and CI of awareness ratings across the event by time point (x-axis) and order of the media (line style). Awareness was rated on a scale ranging from 1 (strongly disagree) to 7 (strongly agree).
